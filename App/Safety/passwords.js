...
export default class Password extends Component {
  ...
  aa(num){
    ...
    if(this.state.info == '请输入旧密码'){
      numbered += num;
      if(length == 4){
        if(Number(this.state.passwords) == Number(numbered)){
          length = 0;
          this.setState({info:'请输入新密码',error:false,})
        }else{
          length = 0;
          numbered = '';
        ...
        <View style={{height:65,paddingTop:20,flexDirection:'row'}}>
             ...
             <View style={{flex:1,alignItems:'center',justifyContent:'center'}}>
                 <View style={{justifyContent:'center',flexDirection:'row',alignItems:'center'}}>
                       <Text style={{color:'white',fontSize:18}} allowFontScaling={false} adjustsFontSizeToFit={false}>更改密码</Text>
                 </View>
             </View>
             <View style={{flex:1,justifyContent:'center'}}>
