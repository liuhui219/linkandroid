...
export default class AllCustomer9 extends React.Component {
    ...
    componentDidMount() {
        //这里获取传递过来的参数: name
		 ...
		 this.fetchData('' + data.data.domain + '/index.php?app=Invoicimg&m=SaleMobile&a=projectlist&option=15&str=&access_token=' + data.data.token + '&p='+this.state.p)
	...
	_ggButton(id){
	 ...
	 renderMovie(data,sectionID: number, rowID: number) {
		if(this.state.sx){
			...
			return (
			   <TouchableOpacity onPress={this.props._selectj.bind(this,data)}>
				<View style={{flexDirection:'row',height:60,alignItems:'center', justifyContent:'space-between',backgroundColor:'#fff',borderBottomWidth:1,borderColor:'#ececec',}}>
				   ...
				   <View style={{flexDirection:'row',marginRight:15, alignSelf:'flex-start',paddingTop:15,}}>
					  <Text allowFontScaling={false} adjustsFontSizeToFit={false} style={{color:'#999'}}>{data.time}</Text>
				   </View>
			   </View>
			   </TouchableOpacity>
				)
	  ...
	  _onRefresh() {
		 this.setState({
		  ...
		  var that=this
				fetch('' + data.data.domain + '/index.php?app=Invoicimg&m=SaleMobile&a=projectlist&option=15&str=&access_token=' + data.data.token + '')
						.then((response) => response.json())
