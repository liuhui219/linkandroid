...
import panainfosc from './panainfosc';
		...
		this.setState({domain:data.data.domain})
         ...
         aa=[];
		  this.timer = setTimeout(
			  () => {this.fetchData('' + data.data.domain + '/index.php?app=Wangpan&m=MobileApi&a=Appaccessory_list&uid='+data.data.uid+'&folder_id='+this.props.dataID.id+'&access_token=' + data.data.token + '');  },
			  500
			);
    }
	_pressButton() {
...
/* 消除空格  end */
	...
	onClose(){this.setState({currentlyOpenSwipeable: null})}
	 renderMovie(data,sectionID: number, rowID: number) {
	  ...
	  fresh(){
		  var that=this;
		  fetch('' + data.data.domain + '/index.php?app=Wangpan&m=MobileApi&a=Appaccessory_list&uid='+data.data.uid+'&folder_id='+this.props.dataID.id+'&file_type=0&access_token=' + data.data.token + '&p='+that.state.p, {
						  method: 'POST',
						  headers: {
							'Content-Type': 'application/x-www-form-urlencoded',
						  }
