		...
		this.setState({domain:data.data.domain})
         ...
         aa=[];
		  this.timer = setTimeout(
			  () => {this.fetchData('' + data.data.domain + '/index.php?app=Wangpan&m=MobileApi&a=lists&uid='+data.data.uid+'&folder_id='+this.props.dataID.id+'&file_type=0&access_token=' + data.data.token + '');  },
			  500
			);
    }
	_pressButton() {
...
/* 消除空格  end */
    ...
    render() {
		...
		return(
		 <View style={{flex:1}}>
		     {this.state.isfalse ? <View style={styles.card}>
					  ...
					  <View style={{flex:1,justifyContent:'flex-end',alignItems:'center', flexDirection:'row'}}>
						   <TouchableOpacity onPress={this.openfile.bind(this)}>
       ...
       <View style={{backgroundColor:'#fff',flex:1}}>
			 ...
			 {this.state.add ? <TouchableOpacity onPress={this._adds.bind(this)}  style={{width:Dimensions.get('window').width,height:Dimensions.get('window').height-45,position:'absolute',top:70,left:0,}}><View style={{width:Dimensions.get('window').width,height:Dimensions.get('window').height-45,backgroundColor:'rgba(61, 61, 62, 0.3)',position:'absolute',top:0,left:0,}}></View></TouchableOpacity> : <View></View>}
					{this.state.add ? <View style={{position:'absolute',top:60,right:5,flexDirection:'column',width:120,height:100,}}>
	...
	onClose(){this.setState({currentlyOpenSwipeable: null})}
	 renderMovie(data,sectionID: number, rowID: number) {
		  ...
		  if(data.icon){
			...
			<View style={{flexDirection:'row'}}>
			  ...
			  <Swipeable rightActionActivationDistance={75} onRightButtonsOpenRelease={this.onOpen.bind(this)} onRightButtonsCloseRelease={this.onClose.bind(this)}
				...
				>
				<TouchableHighlight underlayColor='#ddd' onPress={()=>console.log(1)}>
	  ...
	  fresh(){
		  var that=this;
		  fetch('' + data.data.domain + '/index.php?app=Wangpan&m=MobileApi&a=lists&uid='+data.data.uid+'&folder_id='+this.props.dataID.id+'&file_type=0&access_token=' + data.data.token + '&p='+that.state.p, {
						  method: 'POST',
						  headers: {
							'Content-Type': 'application/x-www-form-urlencoded',
						  }
