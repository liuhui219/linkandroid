...
var array = [];
export default class AllCustomer8 extends React.Component {
    ...
    componentDidMount() {
        //这里获取传递过来的参数: name
		 ...
		 this.fetchData('' + data.data.domain + '/index.php?app=Invoicimg&m=SaleMobile&a=periodlsit&p=1&jump=15&gid='+this.props.id+'&access_token=' + data.data.token + '&p='+this.state.p)
	...
	_ggButton(id){
	 ...
	 renderMovie(data,sectionID: number, rowID: number) {
		if(this.state.sx){
			...
			return (
			   <TouchableOpacity onPress={this.props._selecti.bind(this,data)}>
				<View style={{flexDirection:'row',height:60,alignItems:'center', justifyContent:'space-between',backgroundColor:'#fff',borderBottomWidth:1,borderColor:'#ececec',}}>
				   ...
				   <View style={{flexDirection:'column',flex:1,marginLeft:15}}>
					   <Text allowFontScaling={false} adjustsFontSizeToFit={false} style={{fontSize:18,color:'#777'}}>
						   <HTMLView value={data.number} />
					   </Text>
            ...
            <Text allowFontScaling={false} adjustsFontSizeToFit={false} style={{color:'#999'}}>订单数:</Text>
					  <Text allowFontScaling={false} adjustsFontSizeToFit={false} style={{color:'#999'}}>{data.count}</Text>
				   </View>
			   </View>
			   </TouchableOpacity>
				)
	  ...
	  _onRefresh() {
		 this.setState({
		  ...
		  var that=this
				fetch('' + data.data.domain + '/index.php?app=Invoicimg&m=SaleMobile&a=periodlsit&jump=15&gid='+this.props.id+'&access_token=' + data.data.token + '')
						.then((response) => response.json())
	 ...
	 _onEndReach() {
		  ...
		  if(!this.state.isReach){
			 ...
			 InteractionManager.runAfterInteractions(() => {
				  this.fetchData('' + data.data.domain + '/index.php?app=Invoicimg&m=SaleMobile&a=periodlsit&jump=15&gid='+this.props.id+'&access_token=' + data.data.token + '&p='+this.state.p);
			})
		  }
