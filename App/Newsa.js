...
var aa=[];
var datax=[];
export default class Newsa extends Component {
    ...
    render() {
	 ...
	 _onRefresh() {
		 this.setState({
		  ...
		  var that=this
				fetch('' + data.data.domain + '/index.php?app=Home&m=AuditApi&a=getAudit&uid='+data.data.uid+'&cid='+data.data.cid+'&access_token=' + data.data.token + '&p=1', {
						  method: 'POST',
						  headers: {
							'Content-Type': 'application/x-www-form-urlencoded',
						  },
