...
var array = [];
export default class Scustomerb extends React.Component {
    ...
    componentDidMount() {
        //这里获取传递过来的参数: name
		 ...
		 this.fetchData('' + data.data.domain + '/index.php?app=Legwork&m=MLegwork&a=carcustom&num=15&access_token=' + data.data.token + '&p='+this.state.p)
	 ...
	 renderMovie(data,sectionID: number, rowID: number) {
	  ...
	  _onRefresh() {
		 this.setState({
		  ...
		  var that=this
				fetch('' + data.data.domain + '/index.php?app=Legwork&m=MLegwork&a=carcustom&num=15&access_token=' + data.data.token + '')
						.then((response) => response.json())
