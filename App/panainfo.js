...
/* 消除空格  end */
	...
	onClose(){this.setState({currentlyOpenSwipeable: null})}
	 renderMovie(data,sectionID: number, rowID: number) {
	  ...
	  fresh(){
		  var that=this;
		  fetch('' + data.data.domain + '/index.php?app=Wangpan&m=MobileApi&a=lists&uid='+data.data.uid+'&folder_id=0&file_type=0&access_token=' + data.data.token + '&p='+that.state.p, {
						  method: 'POST',
						  headers: {
							'Content-Type': 'application/x-www-form-urlencoded',
						  }
