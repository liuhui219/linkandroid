...
export default class AllCustomer extends React.Component {
    ...
    componentDidMount() {
        //这里获取传递过来的参数: name
		 ...
		 this.fetchData('' + data.data.domain + '/index.php?app=Account&m=ExpenseApi&a=plist&access_token=' + data.data.token + '&p='+this.state.p)
	 ...
	 renderMovie(data,sectionID: number, rowID: number) {
			...
			return (
			   <TouchableOpacity onPress={this.props._selectxm.bind(this,data)}>
				<View style={{flexDirection:'row',height:60,alignItems:'center', justifyContent:'space-between',backgroundColor:'#fff',borderBottomWidth:1,borderColor:'#ececec',}}>
				   <View style={{marginLeft:5,marginRight:15,backgroundColor:'#dcdcdc',alignItems:'center', justifyContent:'center'}}>
				   ...
				   <View style={{flexDirection:'row',marginRight:15, alignSelf:'flex-start',paddingTop:10,}}>
					  ...
					  <Text allowFontScaling={false} adjustsFontSizeToFit={false} style={{color:'#999'}}>{data.leader}</Text>
				   </View>
			   </View>
			   </TouchableOpacity>
				)
	  ...
	  _onRefresh() {
		 this.setState({
		  ...
		  var that=this
				fetch('' + data.data.domain + '/index.php?app=Account&m=ExpenseApi&a=plist&access_token=' + data.data.token + '')
						.then((response) => response.json())
