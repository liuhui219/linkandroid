...
var array = [];
export default class AllCustomer1 extends React.Component {
    ...
    componentDidMount() {
        //这里获取传递过来的参数: name
		 ...
		 this.fetchData('' + data.data.domain + '/index.php?app=Customer&m=MPrethread&a=lists&num=15&access_token=' + data.data.token + '&p='+this.state.p)
	...
	_ggButton(id){
	 ...
	 renderMovie(data,sectionID: number, rowID: number) {
		if(this.state.sx){
			...
			return (
			   <TouchableOpacity onPress={this.props._selectb.bind(this,data)}>
	  ...
	  _onRefresh() {
		 this.setState({
		  ...
		  var that=this
				fetch('' + data.data.domain + '/index.php?app=Customer&m=MPrethread&a=lists&num=15&access_token=' + data.data.token + '')
						.then((response) => response.json())
