...
export default class Pan extends React.Component {
    ...
    componentDidMount() {
      ...
      this.setState({domain:data.data.domain})
        ...
        this.fetchData('' + this.props.data.data.domain + '/index.php?app=Wangpan&m=MobileApi&a=lists&uid='+this.props.data.data.uid+'&folder_id=0&file_type=0&access_token=' + this.props.data.data.token + '');
      },800);
    }
  ...
  Sent(){
    ...
    this.setState({uploading:true,});
        ...
        fetch('' + that.props.data.data.domain + '/index.php?app=Wangpan&m=MobileApi&a=uploadify&uid='+that.props.data.data.uid+'&fid=0&access_token=' + that.props.data.data.token + '', {
				  method: 'POST',
				  headers: {
     ...
     <View style={{flex:1,backgroundColor:'#fff'}}>
        ...
        <View style={{flex:1,alignItems:'center',justifyContent:'center'}}>
            <View style={{justifyContent:'center',flexDirection:'row',alignItems:'center'}}>
                  <Text style={{color:'#fff',fontSize:18}} allowFontScaling={false} adjustsFontSizeToFit={false}>网盘</Text>
            </View>
        </View>
   ...
   {this.state.uploading ? <View style={{justifyContent: 'center',alignItems: 'center',height:Dimensions.get('window').height,width:Dimensions.get('window').width,position:'absolute',top:0,left:0}}>
    ...
    {this.state.status ? <View style={{backgroundColor:'rgba(119, 119, 119, 0.51)',position:'absolute',width:(Dimensions.get('window').width),height:(Dimensions.get('window').height),top:0,left:0}}><View style={{position:'absolute',backgroundColor:'#fff',width:260,height:150,top:(Dimensions.get('window').height-230)/2,left:(Dimensions.get('window').width-260)/2,borderRadius:5,overflow:'hidden'}}>
     ...
     _onRefresh() {
 	  ...
 	  fresh(){
 		  var that=this;
 		  fetch('' + data.data.domain + '/index.php?app=Wangpan&m=MobileApi&a=lists&uid='+data.data.uid+'&folder_id=0&file_type=0&access_token=' + data.data.token + '', {
 						  method: 'POST',
 						  headers: {
 							'Content-Type': 'application/x-www-form-urlencoded',
 						  }
